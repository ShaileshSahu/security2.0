<!DOCTYPE html>
 <!--[if lt IE 7]>
   <html class="lt-ie9 lt-ie8 lt-ie7" lang="en">
 <![endif]-->

 <!--[if IE 7]>
   <html class="lt-ie9 lt-ie8" lang="en">
 <![endif]-->

 <!--[if IE 8]>
   <html class="lt-ie9" lang="en">
 <![endif]-->

 <!--[if gt IE 8]>
   <!-->
   <html lang="en">
   <!--
 <![endif]-->

 <head>
   <meta charset="utf-8">
   <title>Black Label Admin</title>
   <meta content="width=device-width, initial-scale=1.0, user-scalable=no" name="viewport">

   <link href="icomoon/style.css" rel="stylesheet">
   <!--[if lte IE 7]>
     <script src="css/icomoon-font/lte-ie7.js"></script>
   <![endif]-->
   <link href="css/nvd-Payment.css" rel="stylesheet">
   <link href="css/main.css" rel="stylesheet">
   <link rel="stylesheet" href="css/font.css">

 </head>
 <body>
   <header>
     <a href="index.html" class="logo">Good Year Security Services</a></a>
     <div id="mini-nav">
        <div class="pull-right">
            <a href="viewSearch.php"><button type="button" class="btn btn-success btn-md" name="button">Home</button></a>
        </div>
     </div>
   </header>
   <div class="container-fluid">
     <div id="mainnav" class="hidden-phone hidden-tablet">
       <ul>
         <li>
           <a href="index.html">
             <div class="icon">
               <span class="fs1" aria-hidden="true" data-icon="" class=""></span>
             </div>

           </a>
         </li>
         <li class="active">
           <span class="current-arrow"></span>
           <a href="ViewSearch.php">
             <div class="icon">
               <span  aria-hidden="true" data-icon="&#xf022;" class="fa-list-alt"></span>
             </div>
 View
           </a>
         </li>
         <li>
           <a href="Register.php">
             <div class="icon">
               <span class="fs1" aria-hidden="true" data-icon="&#xe14a;"></span>
             </div>
             Register
           </a>
         </li>
         <li>
           <a href="MasterLogin.php">
             <div class="icon">
               <span class="fs1" aria-hidden="true" data-icon="&#xe0b8;"></span>
             </div>
             Master Login
           </a>
         </li>
         <li>
           <a href="Paymentlogin.php">
             <div class="icon">
               <span class="fs1" aria-hidden="true" data-icon="&#xe0d2;"></span>
             </div>Payment
           </a>
         </li>

         <li>
           <a href="sitename.php">
             <div class="icon">
               <span class="fs1" aria-hidden="true" data-icon="&#xe0b8;"></span>
             </div>
            Modification of Sitename
           </a>
         </li>

         <li>
           <a href="create_monthly_database.php">
             <div class="icon">
               <span class="fs1" aria-hidden="true" data-icon="&#xe0b8;"></span>
             </div>
            Monthly Database
           </a>
         </li>


                  <li>
                    <a href="designation.php">
                      <div class="icon">
                        <span class="fs1" aria-hidden="true" data-icon="&#xe0b8;"></span>
                      </div>
                  Designation
                    </a>
                  </li>
         <li>
           <a href="undo_table.php">
             <div class="icon">
               <span class="fs1" aria-hidden="true" data-icon="&#xe0b8;"></span>
             </div>
            Undo Table
           </a>
         </li>


       </ul>
     </div>

     <div class="dashboard-wrapper">
       <div class="main-container">
         <div class="navbar hidden-desktop">
           <div class="navbar-inner">
             <div class="container">
               <a data-target=".navbar-responsive-collapse" data-toggle="collapse" class="btn btn-navbar">
                 <span class="icon-bar"></span>
                 <span class="icon-bar"></span>
                 <span class="icon-bar"></span>
               </a>
               <div class="nav-collapse collapse navbar-responsive-collapse">
                 <ul class="nav">
                   <li>
                     <a href="View.php">View/Modify</a>
                   </li>
                   <li>
                     <a href="Register.php">Register</a>
                   </li>
                   <li>
                     <a href="Master.php">Master</a>
                   </li>
                   <li>
                     <a href="Paymentlogin.php">Payment</a>
                   </li>

                   <li>
                     <a href="sitename.php">Add of Sitename</a>
                   </li>




                 </ul>
               </div>
             </div>
           </div>
         </div>

         <h1 class="text text-primary" style="color:rgba(0,0,0,.4);text-align:center;">Good Year Security Services</h1>
